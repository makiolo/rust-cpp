...
namespace dp14 {
	...
	public:
		...
		using cache_container = phmap::flat_hash_map<key_cache, std::weak_ptr<T>>;
		using cache_shared_container = phmap::flat_hash_map<key_cache, std::shared_ptr<T>>;
		using registrator_container = phmap::flat_hash_map<key_impl, registrator_function>;
		...
		inline key_cache get_atomic_hash(const key_impl& keyimpl, Args&&... data) const
		{
            key_cache key = keyimpl ^ dp14::hash<T>()(std::forward<Args>(data)...);
            {
		...
		template <typename U>
		std::shared_ptr<T> get(Args... data) const
		{
			key_impl keyimpl = get_key<U>();
			key_cache key = get_atomic_hash(keyimpl, std::forward<Args>(data)...);
	...
	protected:
		std::shared_ptr<T> _get(const key_impl& keyimpl, key_cache key, Args&&... data) const
		{
			...
			{
                // std::cout << "using cache ...: " << keyimpl << std::endl;
